...
// Hook into admin head to modify the logo
/*add_action( 'admin_head', 'custom_plugin_modify_logo' );
function custom_plugin_modify_logo() {
    ?>
    <style>
        #adminmenu #wp-admin-bar-wp-logo > .ab-item .ab-icon:before {
            background-image: url('<?php echo plugins_url( 'images/custom-logo.png', __FILE__ ); ?>') !important;
        }
    </style>
    <?php
}*/
